package com.ozkara.kaan.screens;
...
public class LoginScreen extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPasswordView.setOnEditorActionListener(new TextView.OnEditorActionListener() {
            ...
            public boolean onEditorAction(TextView textView, int id, KeyEvent keyEvent) {
                if (id == R.id.sign_in_short || id == EditorInfo.IME_NULL) {
                    attemptLogin();
                    return true;
                }
                return false;
        ...
        Button mStudentIDsign_in_shortButton = (Button) findViewById(R.id.sign_in_short);
        mStudentIDsign_in_shortButton.setOnClickListener(new OnClickListener() {
            @Override
        ...
        mLoginFormView = findViewById(R.id.sign_in_short);
    ...
    private void populateAutoComplete() {
        ...
        if (VERSION.SDK_INT >= 14) {
            ...
            new SetupstudentIDAutoCompleteTask().execute(null, null);
        }
    }
    ...
    public Loader<Cursor> onCreateLoader(int i, Bundle bundle) {
        return new CursorLoader(this,
                ...
                // Select only studentID addresses.
    ...
    /**
     * Use an AsyncTask to fetch the user's studentID addresses on a background thread, and update
     * the studentID text field with results on the main UI thread.
     */
    class SetupstudentIDAutoCompleteTask extends AsyncTask<Void, Void, List<String>> {

        @Override
        protected List<String> doInBackground(Void... voids) {
            ArrayList<String> studentIDAddressCollection = new ArrayList<>();

            // Get all studentIDs from the user's contacts and copy them to a list.
            ContentResolver cr = getContentResolver();
            Cursor studentIDCur = cr.query(ContactsContract.CommonDataKinds.Email.CONTENT_URI, null,
                    null, null, null);
            while (studentIDCur.moveToNext()) {
                String studentID = studentIDCur.getString(studentIDCur.getColumnIndex(ContactsContract
                        .CommonDataKinds.Identity.IDENTITY));
                studentIDAddressCollection.add(studentID);
            }
            studentIDCur.close();

            return studentIDAddressCollection;
        }

        @Override
        protected void onPostExecute(List<String> studentIDAddressCollection) {
            addstudentIDsToAutoComplete(studentIDAddressCollection);
        }
    }
